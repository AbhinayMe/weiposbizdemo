...
public final class CpuCard {
	...
	public static CmdResult selectFile(IsoDep c, String name) throws CpuCardException {
		byte[] bs=null;
		try {
			bs = name.getBytes("UTF8");
		} catch (Exception e) {
		}
		return selectFile(c,bs);
	}
	
	public static CmdResult selectFile(IsoDep c, byte[] name) throws CpuCardException {
		byte[] bs=name;
